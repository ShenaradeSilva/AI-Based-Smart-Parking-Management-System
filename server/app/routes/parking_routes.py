...
from ..schemas.parking_schema import ParkingSlot, ParkingSlotCreate
...
# Parking Slots
@router.get("/slots/list", response_model=List[ParkingSlot])
...
@router.post("/slots/add", response_model=ParkingSlot)
...
@router.put("/slots/{slot_id}/status-update", response_model=ParkingSlot)
...
def delete_slot(slot_id: int, db: Session = Depends(get_db)):
    success = parking_services.delete_parking_slot(db, slot_id)
    if not success:
        raise HTTPException(status_code=404, detail="Slot not found")
